...
class sketch_line(object):
	...
	def dynamics_draw_trance(self):
		self.parent.Displayshape_core.canva._display.Context.InitDetected();
		...
		#a123=self.parent.Displayshape_core.canva._display.MoveTo(_dragStartPosX,_dragStartPosY)
		ais_kinds=[AIS_Point,AIS_Shape]
		try:
			#SelectMgr_EntityOwner.
			#SelectMgr_SelectableObject
			shape=TopoDS_Shape()
			#aisobj=self.parent.Displayshape_core.canva._display.Context.DetectedInteractive()
			shape=self.parent.Displayshape_core.canva._display.Context.DetectedCurrentObject()#获取高亮的对象
			for kind in ais_kinds:
				try:
					print(kind)
					point = kind.DownCast(shape)
					print("我终于获取到了高亮的",point)
