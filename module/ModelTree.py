...
class ModelTree(QtWidgets.QWidget):
	def __init__(self):
		...
		#self.tree.setStyleSheet("alternate-background-color: yellow;background-color: red;")
...
class parameter_panel(QtWidgets.QWidget):
	def __init__(self,parent):
		super(parameter_panel,self).__init__()
	...
	def set(self):
		width0=self.parent.parameter_items.geometry().width()*0.1
		width1=self.parent.parameter_items.geometry().width()*0.4
		width2=self.parent.parameter_items.geometry().width()*0.4
	...
	def inputdata(self,data={}):
		...
		self.tableWidget.setHorizontalHeaderLabels(["Point ID ", "X","Y"])
		for row in data.keys():
			# 创建SpinBox并将其设置为单元格的小部件
			self.x_spinbox_dict[row]=QDoubleSpinBox()
			self.x_spinbox_dict[row].setRange(-10000, 10000)
			self.x_spinbox_dict[row].setValue(data[row].X())
			self.x_spinbox_dict[row].setDecimals(3)
			self.x_spinbox_dict[row].setSingleStep(0.01)
			self.x_spinbox_dict[row].setStepType(QAbstractSpinBox.AdaptiveDecimalStepType)
			

			self.y_spinbox_dict[row]=QDoubleSpinBox()
			self.y_spinbox_dict[row].setRange(-10000, 10000)
			self.y_spinbox_dict[row].setValue(data[row].Y())
			self.y_spinbox_dict[row].setDecimals(3)
			self.y_spinbox_dict[row].setSingleStep(0.01)
			self.y_spinbox_dict[row].setStepType(QAbstractSpinBox.AdaptiveDecimalStepType)
			
			item = QTableWidgetItem("point"+str(row))
			item.setTextAlignment(Qt.AlignCenter | Qt.AlignBottom)
			self.tableWidget.setItem(row, 0, item)
			self.tableWidget.setCellWidget(row, 1, self.x_spinbox_dict[row])
			self.tableWidget.setCellWidget(row, 2, self.y_spinbox_dict[row])
